	...
	for item in actor.equipped_items:
		for effect in _effects_for_trigger(item, CombatConstantsScript.TRIGGER_HIT):
			...
			if effect.effect_type == CombatConstantsScript.EFFECT_REDUCE_TARGET_ARMOR:
				_mark_effect_fired(actor, effect)
				_reduce_target_armor(log, parent_entry_id, actor, target, item, effect)
			else:
				log.add_child(parent_entry_id, _unsupported_effect_text(actor, item, effect))

static func apply_damaged_item_effects(log, parent_entry_id: int, damaged_unit, attacker) -> void:
	for item in damaged_unit.equipped_items:
		...
		for effect in effects:
			...
			elif effect.effect_type == EFFECT_HEAL or effect.effect_type == CombatConstantsScript.EFFECT_HEAL_SELF:
				_mark_effect_fired(damaged_unit, effect)
				_heal_unit(log, parent_entry_id, damaged_unit, damaged_unit, item, effect)
			else:
	...
	for item in actor.equipped_items:
		for effect in _effects_for_trigger(item, CombatConstantsScript.TRIGGER_KILL):
			if not _effect_can_fire(actor, effect, actor):
				continue
			if effect.effect_type == CombatConstantsScript.EFFECT_HEAL_SELF or effect.effect_type == EFFECT_HEAL:
				_mark_effect_fired(actor, effect)
				_heal_unit(log, parent_entry_id, actor, actor, item, effect)
			else:
				log.add_child(parent_entry_id, _unsupported_effect_text(actor, item, effect))
...
static func _heal_unit(log, parent_entry_id: int, owner, target, item: ItemDefinition, effect) -> void:
...
static func _reduce_target_armor(log, parent_entry_id: int, actor, target, item, effect) -> void:
