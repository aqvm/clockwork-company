...
func _init() -> void:
	...
	var json_elemental_fusion: StatusDefinition = json_content["statuses"].get("elemental_fusion_it", null)
	assert(json_elemental_fusion != null and json_elemental_fusion.status_type == "Elemental Fusion" and json_elemental_fusion.propagation_percent == 25, "JSON should reconstruct composite elemental status payloads.")
	var json_elementalist: JobDefinition = json_content["jobs"].get("elementalist_it", null)
	assert(json_elementalist != null and json_elementalist.secondary_skill.effects[0].effect_type == "Fuse Elemental Ailments" and json_elementalist.secondary_skill.effects[0].amount_divisor == 3, "JSON should reconstruct elemental fusion effects and conversion divisors.")
	assert(json_elementalist.passive.effects[0].trigger == "Owner Applied Ailment" and json_elementalist.reaction.trigger == "Enemy Died With Ailments", "JSON should reconstruct Elementalist passive and reaction triggers.")
	assert(json_elementalist.reaction.effects[0].effect_type == "Transfer Defeated Ailments" and json_elementalist.reaction.effects[0].target_selector == "Most Ailmented Enemy Unit", "JSON should reconstruct defeated-ailment transfer targeting.")
