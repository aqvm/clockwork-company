...
func _init() -> void:
	...
	assert(json_job != null and json_job.skill.action == "Apply Status" and json_job.skill.status != null, "JSON should load job skills and resolve status references.")
	assert(json_job.passive != null and json_job.passive.effects.size() == 1, "JSON should load shared triggered effects on job passives.")
	assert(json_job.passive.effects[0].trigger == "Battle Start" and json_job.passive.effects[0].status.status_type == "Reconstitution", "JSON should preserve triggered-effect fields and status references.")
	assert(json_job.default_tactic != null and json_job.default_tactic.action == "Job Skill", "JSON should load default tactics.")
