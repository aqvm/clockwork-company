...
Between encounters inside a scenario, surviving units should reset to baseline combat capacity. HP, temporary armor, statuses, cooldown-like battle state, and other runtime effects clear before the next fight. The deliberate exception is scenario-local knockout: a unit defeated in one fight remains unavailable for the rest of that scenario.

The first campaign defeat rule should be scenario-local knockout, not long-term death. If a unit is defeated in a fight, that unit becomes unavailable for the remaining fights in the same scenario. After the scenario ends, the unit returns with no long-term penalty. This creates short-term stakes inside multi-fight scenarios without pulling injury, recovery, or replacement systems into the main campaign model.
