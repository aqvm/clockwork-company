...
- campaign planning uses authored tactics as templates, then lets players add/remove/reorder and alter condition/action/target/Foretell fields
- the current job's default tactic remains appended at combat initialization and is not part of the editable loadout list

Foretell is an optional evaluation mode on any normal tactic, gated by an equipped `Forecast` passive. A configured Foretell tactic stays on the unit when that passive is unequipped, but is unavailable until Forecast returns. Foretell follows one deterministic baseline rather than searching alternatives: speculative turns ignore every Foretell toggle and evaluate the complete tactic list as normal condition/action/target rules, the first future state where the configured normal condition is true wins, the normal target selector is evaluated in that speculative state and mapped back to real runtime state, and the action executes now. The horizon ends before the original actor's next turn. Speculation stays presentation-silent, cannot recursively forecast, and uses isolated runtime/Resource copies.
...
Status duration belongs to the application source rather than the status definition. Skills and effects apply statuses for three affected owner turns by default, can author a different finite duration, or can explicitly mark an application permanent. A finite status loses one turn after an owner action only if it was already active at that turn's start, so a self-applied status does not immediately consume duration. Reapplication follows the status definition's stacking rule.
...
Bleed is an action-frequency ailment: after the afflicted unit completes an action, it takes one damage per stack. Bleed intensifies, does not expire naturally, and requires explicit removal. Numb is a short reaction-counter ailment that prevents reaction requests while active. Frost is a physical setup/payoff ailment: its stacks add damage to the next physical damage request against the afflicted unit, then Frost is removed. Shock is an AOE-native magic ailment: one stack converts incoming pre-Energy-Shield magic damage into 25% recursive arcs against up to three deterministic allied targets, with overkill excluded and each discharge consuming a stack. Elemental Fusion is an authorable composite ailment whose shared stack pool follows Burning action damage, Frost physical amplification, and Shock propagation/consumption rules.
