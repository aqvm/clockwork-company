...
- Replace scenario-local knockout's display-name key with a stable campaign unit instance id once recruits can duplicate the same base unit template or unit renaming exists. The current first slice keys knockouts by display name because the starter roster has unique names.
