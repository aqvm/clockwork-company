...
Core `.tres` authoring and its immediate practice-test workflow are documented in `CONTENT_AUTHORING.md`. Scenario Resources saved under `clockwork-company/resources/scenarios/` are automatically discovered for practice; campaign membership remains explicit.
