...
| Scenario rules | `ScenarioRuleDefinition` Resources referenced by scenarios | No mechanical resolver currently; Iron Tollgate is now an authoring note | Scenario detail and Resource tooltips make rules visible before combat |
| Combat state | Simulator-owned `UnitState`, structured events, and snapshots | `CombatSimulator` is authoritative | Logs explain rule outcomes; replay snapshots currently expose HP, timing, alive state, and stable ids |
...
- Scenario rules are deliberately data-first placeholders. A new mechanical rule should be broad, visible, deterministic, and implemented through a focused resolver path only when a scenario needs it.
...
- Statuses, purge rules, scenario-wide mechanical conditions, and broader targeting vocabulary should wait for focused content that needs them.
...
- Replay snapshots intentionally remain small. They do not yet expose temporary armor, ability cooldowns, statuses, or cast state; add those only when a concrete replay visual needs them.
...
- Learned-ability provenance, assignment rules, tactic editing, and broader ability catalogs are not modeled yet. The existing `pending_unlock_choice` boolean and loadout overrides are scaffolding, not a complete progression contract.
...
- define and implement the first authored job unlock choice plus learned-ability assignment
- add status hooks, tooltip support, and replay state only when focused content needs statuses
